...
const INVALID_HANDLE: super::OpaquePtr = 0 as _;
...
impl spec::DynamicLibrary for super::DynamicLibrary {
	type Address = super::OpaquePtr;

	fn load(&mut self, filename: &str) -> spec::Result<()> {
		let handle = unsafe { ffi::dlopen(c_str(filename), ffi::RTLD_LAZY | ffi::RTLD_LOCAL) };
	...
	fn find_symbol(&self, name: &str) -> spec::Result<Self::Address> {
		let symbol_addr = unsafe { ffi::dlsym(self.0, c_str(name)) };
